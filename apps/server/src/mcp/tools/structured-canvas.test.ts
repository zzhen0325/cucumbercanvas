...
import { createStructuredCanvasMcpTools } from "./structured-canvas.js";
...
describe("Cucumber structured canvas canvas MCP tools", () => {
  ...
  it("applies batch_design DSL operations to the live Cucumber canvas", async () => {
    ...
    const result = await server.callTool(
      ...
      {
        operations: [
          'panel=I(null,{type:"frame",name:"Cucumber Panel",x:120,y:160,width:360,height:220,children:[]})',
          'title=I(panel,{type:"text",name:"Title",content:"Copied editing capability",x:144,y:190,width:280,height:48,fontSize:24})',
          'U(title,{content:"Agent-editable Cucumber panel"})',
        ].join("\n"),
      },
      userContext,
    );
    ...
    expect(findNode(harness.state.doc, titleId)).toMatchObject({
      content: "Agent-editable Cucumber panel",
      type: "text",
    });
  });
  ...
  it("manages live canvas pages with Cucumber structured canvas page tools", async () => {
    const harness = createLiveCanvasHarness(createTwoPageDocument());
    const server = createInMemoryMcpServer(
      createStructuredCanvasMcpTools({
        liveCanvasService: harness.liveCanvasService as never,
  ...
  it("supports Cucumber layered canvas design tools on the live requested page", async () => {
    const harness = createLiveCanvasHarness(createTwoPageDocument());
    const server = createInMemoryMcpServer(
      createStructuredCanvasMcpTools({
        liveCanvasService: harness.liveCanvasService as never,
  ...
  it("supports Cucumber style ops and variables against live documents", async () => {
    const doc = createEmptyDocument();
    const children: PenDocument["children"] = [
      {
        id: "frame-style",
  ...
  it("stores and assembles Cucumber codegen chunks", async () => {
    const harness = createLiveCanvasHarness(createEmptyDocument());
    const server = createInMemoryMcpServer(
      createStructuredCanvasMcpTools({
        liveCanvasService: harness.liveCanvasService as never,
