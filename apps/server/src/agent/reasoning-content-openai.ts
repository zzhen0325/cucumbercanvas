...
type ChatCompletionMessageParamWithReasoning = Record<string, unknown> & {
...
/**
 * LangChain preserves OpenAI-compatible provider `reasoning_content` on inbound
 * AIMessage.additional_kwargs, but Chat Completions message conversion does not
 * currently write it back into assistant history messages. Some thinking-mode
 * providers reject the next tool-round request unless this field round-trips.
 */
export function convertMessagesToReasoningContentCompletionsParams(options: {
  messages: BaseMessage[];
  model?: string;
}): ChatCompletionMessageParamWithReasoning[] {
  let patchedCount = 0;
  const converted = options.messages.flatMap((message) => {
    const mapped = convertMessagesToCompletionsMessageParams({
      messages: [message],
      ...(options.model ? { model: options.model } : {}),
    }) as ChatCompletionMessageParamWithReasoning[];

    const reasoningContent = readReasoningContent(message);
    if (!reasoningContent) return mapped;
  ...
  if (patchedCount > 0) {
    console.info("[model] preserved reasoning_content in assistant history", {
      messagesPatched: patchedCount,
      model: options.model,
    });
  }
...
export function createReasoningContentChatOpenAI(
...
class ReasoningContentChatOpenAICompletions extends ChatOpenAICompletions<ChatOpenAICompletionsCallOptions> {
  async _generate(
    messages: BaseMessage[],
  ...
  async *_streamResponseChunks(
    ...
    for await (const rawData of streamIterable) {
      ...
      if (typeof chunk.content !== "string") {
        console.warn(
          "[model] Received non-string content from OpenAI-compatible stream.",
        );
        continue;
      }
...
function readReasoningContent(message: BaseMessage): string | null {
