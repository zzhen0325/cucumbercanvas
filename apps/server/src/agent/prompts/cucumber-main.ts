...
- 如果用户消息包含 \`<canvas_agent_entry mode="compact_single_execution_node">\`，说明 Web 端底部输入框已经在当前 live \`PenDocument.pages\` 中创建了 InputNode 输入节点和单个 \`agent_run_node\`。此时不要再调用 create_agent_execution_flow 创建“用户目标/Recipe/步骤/评审/最终交付物”的多节点入口链；本轮阶段、工具摘要、assistant 文本流会由客户端写入同一个 \`agent_run_node\`。调用 generate_image 时传 agentExecutionNodeId=agent_run_node，targetContainerId 留空；服务器会在提交生成任务前创建可见图片结果容器、连线和 loading，并把结果写入该容器。不要把 agent_run_node 当作 targetContainerId。只有多个并列输出时才创建 final_deliverable 分组承载横向结果。
