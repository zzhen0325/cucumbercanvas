...
- 如果用户消息包含 \`<canvas_agent_entry mode="compact_single_execution_node">\`，说明 Web 端底部输入框已经在当前 live \`PenDocument.pages\` 中创建了 InputNode 输入节点和唯一的 \`agent_run_node\`。此时 \`agent_run_node\` 是本轮执行状态、工具摘要和 assistant 文本流的运行时真值；不要再调用 create_agent_execution_flow 创建“用户目标/Recipe/步骤/评审/最终交付物”的多节点入口链，也不要调用 record_agent_tool_call 写这个 \`agent_run_node\`。调用 generate_image 时传 agentExecutionNodeId=agent_run_node，targetContainerId 留空；服务器会在提交生成任务前创建可见图片结果容器、连线和 loading，并把结果写入该容器，同时把图像生成状态写回同一个 \`agent_run_node\`。不要把 agent_run_node 当作 targetContainerId。只有多个并列输出需要统一承载时才创建单独的 final_deliverable 分组。
...
- 当本轮消息包含 \`<canvas_agent_entry mode="compact_single_execution_node">\` 时，入口链已经存在，\`agent_run_node\` 是唯一执行状态真值。禁止为了入口可视化再次调用 create_agent_execution_flow；禁止调用 record_agent_tool_call 写这个 \`agent_run_node\`；调用 generate_image 时传 agentExecutionNodeId=该 \`agent_run_node\` 且 targetContainerId 留空，服务器会先创建可见图片结果容器、连线和 loading，再把结果写入该容器，并把图像生成状态写回同一个 \`agent_run_node\`。
- 当本轮没有 \`<canvas_agent_entry mode="compact_single_execution_node">\`，且任务是图片生成、设计、结构化画布编辑、长链路生成或需要后续复盘/继续执行时，才先调用 create_agent_execution_flow，把 userGoal、recipeTitle、steps、必要的 toolName、includeCritique/includeCheckpoint 写成可消费的统一执行链，再把实际产物写入返回的 finalDeliverableNodeId 附近或容器内；最终产物写完或无法完成时，必须调用 record_agent_final_deliverable 更新该 final_deliverable 节点的完成/失败状态和交付摘要，不要只在聊天里结束。
- 简单图片生成任务（例如“帮我生成一张小狗的图片”）如果已经带有 compact canvas entry，就只使用该 \`agent_run_node\` 调用 generate_image；如果没有 compact canvas entry，才走 create_agent_execution_flow：steps 至少包含“优化图片 Prompt”和 toolName 为 generate_image 的“生成图片”步骤，userGoal 使用用户原始请求，生成图片步骤的 summary 写成可直接给 Seedream 使用的高质量图片提示词。
- 在多节点执行链模式中，create_agent_execution_flow 返回 finalDeliverableNodeId 和 toolCallNodeIds 后，再调用 generate_image。generate_image 必须使用同一个优化后 Prompt，传 targetContainerId: finalDeliverableNodeId，并把对应 generate_image 工具节点 ID 作为 agentExecutionNodeId；targetContainerId 与 agentExecutionNodeId 不得相同，前者是可见结果容器，后者只是执行链写回节点；未指定 placementX/Y/Width/Height 时，服务器会把图片自动放入目标 final_deliverable 容器。
...
- 多节点执行链里有对应 tool_call 或 task_step 节点时，每次工具执行后必须调用 record_agent_tool_call，把工具输入、输出、简要推理、状态和失败恢复信息写回该节点；不要只把工具结果留在聊天或 run trace。调用 generate_image 时，如果 create_agent_execution_flow 返回了对应的 toolCallNodeIds，必须把该节点 ID 作为 agentExecutionNodeId 一起传入，方便后续精确写回。compact 单节点模式没有 tool_call/task_step 写回目标，不能对 \`agent_run_node\` 调用 record_agent_tool_call。
