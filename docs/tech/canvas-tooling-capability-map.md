...
- `meta.agentExecution`: current Agent execution canvas metadata, schema version 1. It is the durable UI/Agent truth for Flowith-like execution-node semantics such as `user_goal`, `recipe_plan`, `tool_call`, `critique`, `ask_user_more`, `checkpoint`, `variant_branch`, `comparison`, and `final_deliverable`, plus upstream/downstream chain links, node-level detail summaries, structured critique findings, failure context, branch mainline/recommendation choice, and user response text for blocking `ask_user_more` nodes. It lives on the same `PenNode` that renders on the canvas and is not a separate run-trace fallback.
- `withAgentExecutionNodeSemantics` in `packages/canvas-core/src/agent-execution.ts` is the shared write boundary for newly created execution nodes that need both durable `meta.agentExecution` and top-level semantic indexes such as `runId`, `sessionId`, `agentBinding`, `containerRole`, and `contextSlots`. The simple flow, generic execution flow, ask-user-more, evidence, and variant/comparison creation tools use this boundary instead of hand-writing partial execution metadata in separate paths.
...
- Simple image-generation execution nodes created by `create_agent_canvas_flow` now write `meta.agentExecution` together with top-level `runId`, `sessionId`, `agentBinding`, non-empty `containerRole`, and execution `contextSlots`, including the nested loading `tool_call` nodes inside the result container. The duplicated top-level fields are semantic/read/trace indexes over the same `PenNode`, not a second execution truth.
...
- The canvas-level Agent run control bar derives its durable context from selected `PenNode.meta.agentExecution` plus the active run ID reported by chat streaming. It does not create a second canvas execution truth.
...
Recipe template UI state:

- Built-in Recipe starters are defined in `packages/canvas-core/src/agent-recipe-template.ts`, not in a separate persisted Agent state store. The chat input can select a template, prefill an editable user prompt, show a Recipe chip, and send `<agent_recipe_template>` as prompt context.
- Template context describes intended `meta.agentExecution` node structure, tool order, input slots, validation rules, and deliverable format. The Agent must still materialize the actual chain through durable `PenDocument.pages` nodes and existing MCP tools such as `create_agent_execution_flow` and `create_agent_variant_branches`.
- Completed `recipe_plan`, `comparison`, `checkpoint`, and `final_deliverable` execution nodes expose `保存为 Recipe 模板` in the property panel. The saved template is extracted from the selected node's durable `meta.agentExecution` metadata plus related completed upstream/downstream execution nodes on the active page, then stored in browser local storage for the ChatInput Recipe menu. This is reusable workflow configuration, not canvas runtime truth.
- Saved local Recipe templates can be previewed in the chat input menu for node structure, tool order, input slots, and source node IDs, and can be removed from local storage. Removing a saved template does not mutate or delete any `PenDocument.pages` execution nodes.
...
| `create_agent_execution_flow` | Yes | Creates a generic durable Agent execution chain for complex tasks: user goal, Recipe, task steps, optional tool-call nodes, critique, final deliverable, checkpoint, and semantic connectors, all tagged with `meta.agentExecution` upstream/downstream links for UI inspection, Recipe extraction, and future continuation controls. |
...
| `create_agent_canvas_flow` | Yes | Creates the current simple image-generation execution chain and tags its durable nodes with `meta.agentExecution` roles (`user_goal`, `recipe_plan`, `tool_call`, `final_deliverable`) so the property panel and chat input can treat selected nodes as continuation context. `generate_image` can carry `agentExecutionNodeId` to correlate image jobs/results with a durable tool-call node; the runtime best-effort writes terminal image job status/details back to that node, and `record_agent_tool_call` remains the explicit manual write-back boundary for other tool results. |
