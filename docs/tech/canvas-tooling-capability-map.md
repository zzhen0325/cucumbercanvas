...
- Expanded `agent_run_node` details render through the React AgentRunNode overlay. Skia keeps only the lightweight durable frame shell for this node kind, while the overlay follows the same transient transform preview used by Skia during drag/resize/rotate. Autosize write-back is limited to mounted expanded AgentRunNode content so collapsed shells do not let DOM measurement silently drive canvas bounds.
