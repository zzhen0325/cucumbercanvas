...
| `create_agent_execution_flow` | Yes | Creates the unified durable Agent execution chain for image generation, design, structured canvas editing, and continuation-oriented tasks: user goal, Recipe, task steps, optional tool-call nodes, critique, final deliverable, checkpoint, and semantic connectors, all tagged with `meta.agentExecution` upstream/downstream links for UI inspection, Recipe extraction, and future continuation controls. When a step uses `generate_image`, the final deliverable is sized as a visual result container and `generate_image` should target that node. |
