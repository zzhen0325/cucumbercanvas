...
- Added `scripts/prepare-visual-canvas.mjs` and `pnpm prepare:visual-canvas` so local Browser visual acceptance can create or reuse a real project through `/api/projects`, verify the returned `primaryCanvas.id` through `/api/canvases/:canvasId`, and print a usable `/canvas?id=<uuid>` URL instead of relying on the fake `canvas-1` test id.
- Kept canvas runtime fail-fast behavior unchanged: missing canvases still return `Canvas not found`, and local fixture creation stays at the API/data-preparation boundary rather than becoming a hidden runtime fallback.
- Updated the transport test harness to use a dedicated configurable `transport-canvas-fixture` id via `?canvasId=` instead of spreading `canvas-1` as a default outside mocked tests.
