> **[STAR]**
> **本文档定位**
> 本文档是 CucumberCanvas 改造的**唯一启动参考文档**，整合了三份子方案（多角色容器系统架构、Headless Engine + Pixi.js v8 引擎改造、OpenPencil Agent 系统复用评估）。它解决一个核心问题——**把 CucumberCanvas 从"AI 画板工具"升级为"AI 原生无限画布"**，并直接作为 P0 编码启动、issue 拆分、跨团队对齐的依据。
> 阅读建议：先看「关键决策摘要」对齐选型，再看「统一总架构图」建立全局直觉，然后按「分阶段实施路径」拆 issue 即可启动 P0。

## 零、产品定位与核心原则

Cucumber Studio 的核心定位是 **AI 原生无限画布**：画布不是先存在的空白空间，而是 Agent 执行过程的可视化产物；每个容器不是用户手动创建的普通元素，而是 Agent 思考、决策和输出的自然结果。

三大核心原则：

- **Agent 优先**：所有主要内容由 Agent 根据用户目标、反馈和当前画布上下文生成；用户可以在结果上继续编排、微调，也可以把修改后的状态交给 Agent 继续执行。
- **容器即输出**：每个有明确边界的 Agent 执行结果，都应以独立容器呈现在画布上，承载任务阶段、产物边界、上下文分区或数据流节点。
- **空间即上下文**：容器的位置、大小、分组、嵌套和连接关系天然表达 Agent 的思考逻辑、依赖关系、上下游数据流和后续可操作区域。

基础手动编辑能力保留，定位为用户对 Agent 结果的编排、修订和反馈入口；不把画布重新定义为空白设计工具，也不把手动创建容器作为主要生产路径。

### 0.1 典型使用场景

**场景一：图像生成后的二次修改**

用户输入目标后，Agent 先分析需求、选择生成策略，并把最终图像作为画布上的结果容器呈现。用户点击生成结果后，应进入“围绕该结果继续修改”的上下文：可以在 Agent 对话浮层里输入新指令，也可以直接使用浮层工具条上的高频动作，例如高清放大、扩图、局部编辑等。这些快捷动作本质上不是独立工具按钮，而是带预设意图的 Agent 二次执行入口，应该复用当前选中结果、原始提示、生成参数和画布上下文。

**场景二：PPT / 网页 / UI 等结构化产物**

当 Agent 生成 PPT、网页或 UI 界面时，画布结果不只是单张图片，而是可继续编辑的结构化设计稿。此类产物需要更强的自定义编辑能力，包括元素级选择、文本编辑、组件/Frame 管理、样式面板、图层、对齐、吸附、导入导出等，整体体验可以参考项目内 `openpencil/` 的方向：Prompt to Canvas、选中元素后聊天修改、空间化子任务拆解、headless engine、React 编辑器 SDK、Figma 导入和 Design-as-Code 导出。

这两类场景共享同一个原则：Agent 负责生成第一版结构化结果，用户通过上下文浮层、快捷动作和高级编辑能力完成二次修改；所有修改后的状态继续成为下一轮 Agent 执行上下文。
