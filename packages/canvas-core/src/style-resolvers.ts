...
export function resolveFillCSS(fills?: CanvasFill[]): CSSProperties {
  ...
  const visibleFills = fills.filter((f) => {
    ...
    if ("opacity" in f && f.opacity !== undefined && f.opacity <= 0)
      return false;
    return true;
  });
  ...
  for (const fill of visibleFills) {
    ...
    switch (fill.type) {
      case "solid": {
        const color =
          opacity !== undefined
            ? applyOpacity(fill.color, opacity)
            : fill.color;
      ...
      case "radial_gradient": {
        ...
        const stops = fill.stops
          ...
          .join(", ");
        backgrounds.push(
          `radial-gradient(circle at ${cx * 100}% ${cy * 100}%, ${stops})`,
        );
        break;
      }
      case "angular_gradient": {
        ...
        const stops = fill.stops
          ...
          .join(", ");
        backgrounds.push(
          `conic-gradient(from ${fill.angle ?? 0}deg at ${cx * 100}% ${cy * 100}%, ${stops})`,
        );
        break;
      }
      case "diamond_gradient": {
        const stops = fill.stops
          .map((s) => `${s.color} ${Math.round(s.offset * 100)}%`)
          .join(", ");
        backgrounds.push(`radial-gradient(closest-side, ${stops})`);
        break;
      }
      case "image": {
...
export function resolveStrokeCSS(stroke?: CanvasStroke): CSSProperties {
  ...
  const color =
    stroke.fill?.[0] && stroke.fill[0].type === "solid"
      ? stroke.fill[0].color
      : "transparent";
...
export function resolveEffectCSS(effects?: CanvasEffect[]): CSSProperties {
  ...
  for (const effect of effects) {
    if (
      ...
      (effect.opacity !== undefined && effect.opacity <= 0)
    ) {
      continue;
    }
    switch (effect.type) {
