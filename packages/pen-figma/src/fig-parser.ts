...
import type { FigmaDecodedFile } from "./figma-types.js";
...
function figToBinaryParts(fileBuffer: ArrayBuffer): FigBinaryResult {
    ...
    for (const [path, bytes] of Object.entries(unzipped)) {
      ...
      if (path.startsWith("images/") && bytes.length > 0) {
        if (bytes.length > MAX_IMAGE_SIZE) {
          throw new Error(
            `Image exceeds maximum size limit (${formatMiB(MAX_IMAGE_SIZE)})`,
          );
        }
  ...
  if (!hasFigKiwiMagic(fileByte)) {
    throw new Error(
      "Invalid .fig file: missing fig-kiwi header after extraction",
    );
  }
