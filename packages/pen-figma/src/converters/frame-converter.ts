...
export function setConvertChildren(
  fn: (parent: TreeNode, ctx: ConversionContext) => PenNode[],
) {
  _convertChildren = fn;
}
...
function applyInstanceOverrides(
  ...
  const len1Derived = safeDerived.filter(
    (d) => d.guidPath?.guids?.length === 1,
  );
  ...
  function applyToNode(node: TreeNode): TreeNode {
    ...
    if (d) {
      ...
      if (d.derivedTextData?.characters !== undefined)
        figma.textData = d.derivedTextData;
    }
