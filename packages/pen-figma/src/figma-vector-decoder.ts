...
function r(n: number): string {
  return Math.abs(n) < 0.00005
    ? "0"
    : Number.parseFloat(n.toFixed(4)).toString();
}
...
function decodeVectorNetworkBlob(
  ...
  try {
    ...
    offset += 4;
    if (vertexCount > 100000 || offset + vertexCount * 8 > blob.length)
      return null;
...
function emitSegment(
  ...
  if (isStraight) {
    ...
    parts.push(
      `C${r(cp1x)} ${r(cp1y)} ${r(cp2x)} ${r(cp2y)} ${r(ev.x * sx)} ${r(ev.y * sy)}`,
    );
  }
}
