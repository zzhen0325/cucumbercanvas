...
export class SpatialIndex {
  ...
  /**
   * Incrementally update or insert a single render node. Avoids full rebuild
   * during per-frame interactions like anchor dragging or live transforms.
   */
  upsertNode(rn: RenderNode, zIndex: number) {
    const existing = this.items.get(rn.node.id);
    if (existing) {
  ...
  /**
   * Remove a single node from the index. Call when a node is deleted from
   * the document without a full rebuild.
   */
