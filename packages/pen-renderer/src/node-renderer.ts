...
import type { PenFill, PenStroke, PenEffect, ShadowEffect, ImageFill, GradientStop } from '@cucumber/pen-types';
import {
  DEFAULT_FILL,
  DEFAULT_STROKE,
...
export class SkiaNodeRenderer {
  ...
  private makeFillPaint(
    ...
    if (first.type === 'solid') {
      ...
      if (stops.length >= 2) {
        ...
        const positions = stops.map((s: GradientStop) => Math.max(0, Math.min(1, s.offset)));
  ...
  drawNode(canvas: Canvas, rn: RenderNode) {
    ...
    // Apply blur effect via saveLayer with ImageFilter
    let blurApplied = false;
    const blurEffect = effects?.find((e): e is import('@cucumber/pen-types').BlurEffect => e.type === 'blur' || e.type === 'background_blur');
    if (blurEffect && blurEffect.radius > 0) {
      canvas.save();
      blurApplied = true;
      const blurPaint = new ck.Paint();
      blurPaint.setImageFilter(ck.ImageFilter.MakeBlur(blurEffect.radius / 2, blurEffect.radius / 2, ck.TileMode.Clamp, null));
  ...
  private drawPath(
    ...
    const bounds = geometryBounds
      ...
      : path.getBounds();
    const b2 = bounds[2]!, b0 = bounds[0]!, b3 = bounds[3]!, b1 = bounds[1]!;
    const nativeW = b2 - b0,
      nativeH = b3 - b1;
