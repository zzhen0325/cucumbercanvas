...
import { diffDocuments } from "../merge/node-diff.js";
...
describe("diffDocuments", () => {
  ...
  it("produces add + modify + remove in a single mixed diff", () => {
    ...
    const patches = diffDocuments(before, after);
    expect(
      patches.find((p) => p.op === "modify" && p.nodeId === "a"),
    ).toBeDefined();
    expect(
      patches.find((p) => p.op === "remove" && p.nodeId === "b"),
    ).toBeDefined();
    expect(
      patches.find((p) => p.op === "add" && p.nodeId === "c"),
    ).toBeDefined();
