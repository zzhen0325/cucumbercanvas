...
describe("text-measure", () => {
  ...
  describe("CJK detection", () => {
    it("detects CJK code points", () => {
      const chineseCodePoint = "中".codePointAt(0);
      const japaneseCodePoint = "あ".codePointAt(0);
      const latinCodePoint = "A".codePointAt(0);
      expect(chineseCodePoint).toBeDefined();
      expect(japaneseCodePoint).toBeDefined();
      expect(latinCodePoint).toBeDefined();
      if (
        chineseCodePoint === undefined ||
        japaneseCodePoint === undefined ||
        latinCodePoint === undefined
      ) {
        throw new Error("Test code points must be defined");
      }
      expect(isCjkCodePoint(chineseCodePoint)).toBe(true);
      expect(isCjkCodePoint(japaneseCodePoint)).toBe(true);
      expect(isCjkCodePoint(latinCodePoint)).toBe(false);
    });
  ...
  describe("estimateTextHeight", () => {
    ...
    it("estimates taller for multi-line text", () => {
      ...
      expect(estimateTextHeight(multi)).toBeGreaterThan(
        estimateTextHeight(single),
      );
    });
  });
});
