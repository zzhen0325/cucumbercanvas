...
describe("tree-utils", () => {
  ...
  describe("updateNodeInTree", () => {
    ...
    it("reuses untouched branches when updating a nested node", () => {
      ...
      expect(
        (result[0] as PenNode & { children: PenNode[] }).children[0].name,
      ).toBe("updated");
  ...
  describe("page helpers", () => {
    ...
    it("rejects documents without pages", () => {
      ...
      expect(() => getActivePageChildren(legacy, DEFAULT_PAGE_ID)).toThrow(
        "PenDocument must include non-empty pages.",
      );
    });

    it("preserves existing pages when setting active children", () => {
      const doc = createEmptyDocument();
      const updated = setActivePageChildren(doc, DEFAULT_PAGE_ID, [
        rect("kept"),
      ]);
      expect(updated.pages?.[0].children[0].id).toBe("kept");
    });
  });
});
