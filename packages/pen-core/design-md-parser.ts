...
// ---------------------------------------------------------------------------
// Local type definitions (not exported by @cucumber/pen-types)
// ---------------------------------------------------------------------------
...
const SECTION_PATTERNS: Record<string, RegExp> = {
  ...
  generationNotes:
    /generation\s*notes?|design\s*system\s*notes?|stitch|prompting/i,
};
...
function parseColors(content: string): DesignMdColor[] {
  ...
  for (const line of lines) {
    ...
    // Try pattern: - name: #hex (role)  OR  - name: #hex — role
    const colonHexMatch = line.match(
      /[-*]\s*([^:#(]+?)\s*:\s*#?[0-9A-Fa-f]{6}\s*\(?([^)]*)\)?/,
    );
    if (colonHexMatch) {
      const name = colonHexMatch[1]?.trim();
      const role = colonHexMatch[2]?.trim();
      if (!name || !role) continue;
      colors.push({
        name,
        hex,
    ...
    const before = line
      .substring(0, hexIndex)
      .replace(/[-*#\s]+$/, "")
      .trim();
    const after = line
      .substring(hexIndex + hexMatch[0].length)
      .replace(/^[)\s–—:-]+/, "")
      .trim();
    colors.push({
      name: before || hex,
...
export function extractDesignMdFromDocument(doc: PenDocument): DesignMdSpec {
  ...
  const collectFonts = (
    ...
    for (const n of nodes) {
      if ("fontFamily" in n && typeof n.fontFamily === "string")
        fonts.add(n.fontFamily);
      if ("children" in n && Array.isArray(n.children))
        collectFonts(
          n.children as { fontFamily?: string; children?: unknown[] }[],
        );
    }
  };
  ...
  if (doc.pages) {
    for (const page of doc.pages) {
      collectFonts(
        page.children as { fontFamily?: string; children?: unknown[] }[],
      );
    }
  }
